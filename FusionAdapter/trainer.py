...
from Adaptor_image import WayGAN1
from Adaptor_text import WayGAN2
...
class Trainer:
    def __init__(self, data_loaders, dataset, parameter):
        ...
        self.adaptor_image = WayGAN1(parameter).getVariables1().cuda()
        self.adaptor_text = WayGAN2(parameter).getVariables2().cuda()

        self.metaR.to(self.device)
        self.adaptor_image.to(self.device)
        self.adaptor_text.to(self.device)
        ...
        self.optimizer_adapter = torch.optim.Adam([
            {'params': self.adaptor_image.parameters(), 'lr': 0.0001},
            {'params': self.adaptor_text.parameters(), 'lr': 0.0001},
            # Adapter parameters
        ])
    ...
    def reload(self):
        if self.parameter['eval_ckpt'] is not None:
            ...
            state_dict_adaptor_image_file = os.path.join(self.state_dir, 'state_dict_adaptor_image')
            state_dict_adaptor_text_file = os.path.join(self.state_dir, 'state_dict_adaptor_text')

        self.state_dict_file = state_dict_file
        self.state_dict_adaptor_text_file = state_dict_adaptor_text_file
        self.state_dict_adaptor_image_file = state_dict_adaptor_image_file
        ...
        state_adaptor_text = torch.load(state_dict_adaptor_text_file, map_location=self.device)
        state_adaptor_image = torch.load(state_dict_adaptor_image_file, map_location=self.device)

        if os.path.isfile(state_dict_file):
            self.metaR.load_state_dict(state)
            self.adaptor_image.load_state_dict(state_adaptor_image)
            self.adaptor_text.load_state_dict(state_adaptor_text)
        else:
    ...
    def save_checkpoint(self, epoch):
        ...
        torch.save(self.adaptor_image.state_dict(), os.path.join(self.ckpt_dir, 'state_dict_adaptor_image_' + str(epoch) + '.ckpt'))
        torch.save(self.adaptor_text.state_dict(), os.path.join(self.ckpt_dir, 'state_dict_adaptor_text_' + str(epoch) + '.ckpt'))
    ...
    def save_best_state_dict(self, best_epoch):
        ...
        shutil.copy(os.path.join(self.ckpt_dir, 'state_dict_adaptor_image_' + str(best_epoch) + '.ckpt'),
                    os.path.join(self.state_dir, 'state_dict_adaptor_image'))
        shutil.copy(os.path.join(self.ckpt_dir, 'state_dict_adaptor_text_' + str(best_epoch) + '.ckpt'),
                    os.path.join(self.state_dir, 'state_dict_adaptor_text'))
    ...
    def contrastive_loss_with_negatives(self,
            support_image, support_text, support_structural,
            support_negative_image, support_negative_text, support_negative_structural,
            margin=0, lambda_neg=0.1):

        # Positive Pair Loss (align within the support set)
        image_sim_pos = F.cosine_similarity(support_image, support_structural, dim=-1)
        text_sim_pos = F.cosine_similarity(support_text, support_structural, dim=-1)

        #positive_loss = (1 - image_sim_pos.mean()) + (1 - text_sim_pos.mean())
        positive_loss = F.relu(image_sim_pos - margin).mean() + F.relu(text_sim_pos - margin).mean()

        # Negative Pair Loss (distant from negative samples)
        #image_sim_neg = F.cosine_similarity(support_image, support_negative_image, dim=-1)
        #text_sim_neg = F.cosine_similarity(support_text, support_negative_text, dim=-1)

        #negative_loss = F.relu(image_sim_neg - margin).mean() + F.relu(text_sim_neg - margin).mean()

        # Combine positive and negative loss
        #loss = positive_loss + lambda_neg * negative_loss
        loss = positive_loss
        return loss

    def do_one_step(self, task, adaptor_image, adapter_text, iseval=False, curr_rel=''):
        loss, p_score, n_score = 0, 0, 0
        if not iseval:
            self.optimizer.zero_grad()
            self.optimizer_adapter.zero_grad()
            p_score, n_score, support_structural, support_negative_structural, support_image, support_negative_image, \
            support_text, support_negative_text= self.metaR(task, adaptor_image, adapter_text, iseval, curr_rel)
            y = torch.Tensor([1]).to(self.device)
            loss = self.metaR.loss_func(p_score, n_score, y)

            loss_align = self.contrastive_loss_with_negatives(
                support_image=support_image,
                support_text=support_text,
                support_structural=support_structural,
                support_negative_image=support_negative_image,
                support_negative_text=support_negative_text,
                support_negative_structural=support_negative_structural
            )
            ...
            p_score, n_score , _, _, _, _, \
            _, _= self.metaR(task,adaptor_image, adapter_text,  iseval, curr_rel)
            y = torch.Tensor([1]).to(self.device)
            loss = self.metaR.loss_func(p_score, n_score, y)
        return loss, p_score, n_score


    def task_training(self, best_value,epoch, best_epoch,task, adaptor_image,adapter_text,iseval=False, curr_rel=''):
        loss, p_score, n_score = 0, 0, 0
        self.optimizer_adapter.zero_grad()
        p_score, n_score = self.metaR.task_adaptor(task, adaptor_image,adapter_text, iseval, curr_rel)
        y = torch.Tensor([1]).to(self.device)
        loss = self.metaR.loss_func(p_score, n_score, y)
        loss.backward()
        self.optimizer_adapter.step()
    ...
    def train(self):
        ...
        for e in range(self.epoch):
            ...
            loss, _, _ = self.do_one_step(train_task, self.adaptor_image,self.adaptor_text,iseval=False, curr_rel=curr_rel)
    ...
    def eval(self, istest=False, epoch=None):
        ...
        while True:
            ...
            _, p_score, n_score = self.do_one_step(eval_task,self.adaptor_image,self.adaptor_text, iseval=True, curr_rel=curr_rel)
    ...
    def eval_by_relation(self, istest=False, epoch=None):
        ...
        for rel in data_loader.all_rels:
            ...
            if self.metatrain_adaptor == 'False':
                nn.init.xavier_normal_(self.adaptor_image.MLP1.fc.weight)
                nn.init.xavier_normal_(self.adaptor_text.MLP1.fc.weight)
                nn.init.xavier_normal_(self.adaptor_image.MLP1.fc1.weight)
                nn.init.xavier_normal_(self.adaptor_text.MLP1.fc1.weight)
            ...
            #Train adaptor on meta-test few-shot
            eval_task_support, curr_rel = data_loader.next_one_on_eval_by_relation_support(rel)
            for epoch in range(10):
                self.task_training(best_value, epoch, best_epoch, eval_task_support, self.adaptor_image,self.adaptor_text,
            ...
            while True:
                ...
                _, p_score, n_score = self.do_one_step(eval_task,self.adaptor_image,self.adaptor_text,iseval=True, curr_rel=rel)
                x = torch.cat([n_score, p_score], 1).squeeze()
